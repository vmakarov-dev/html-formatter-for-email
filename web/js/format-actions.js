  ...
  // Вызывается ТОЛЬКО из acceptSuggestion/acceptDeletion, когда именно
  // ПРИНЯТИЕ подсказки исчерпало workingTags (см. там же — reject
  // ВСЕГДА зовёт renderOutput() напрямую, никогда эту функцию: если
  // содержимое не менялось, переформатировать нечего, а повторный разбор
  // без памяти об отклонении тут же нашёл бы тот же дефект заново и
  // "воскресил" бы только что отклонённую подсказку — реальный баг,
  // из-за которого клик по "✕" на последней подсказке выглядел так, будто
  // ничего не произошло). Строчечная вставка/сдвиг номеров строк в
  // accept* — расчёт "на глаз", без полного повторного разбора дерева;
  // раз содержимое реально изменилось и решать больше нечего, прогоняем
  // накопленный lastCleanHtml через форматтер ещё раз с нуля — это даёт
  // настоящий, а не приближённый отступ по всему документу и заодно
  // вскрывает проблемы, которые могли обнаружиться только ПОСЛЕ принятия
  // предыдущих подсказок (например, вложенный дальше тег, чей родитель
  // появился только что).
